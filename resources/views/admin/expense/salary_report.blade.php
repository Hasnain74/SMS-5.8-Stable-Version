...
@section('content')
		...
		<div class="ml-4">
			...
			<div class="row">
				<div class="col-md-12 text-center">
					...
					<h1><i class="fas fa-file-invoice-dollar"></i> Salary Report</h1>
				</div>
			</div>
		...
		<div class="filterable">
			...
			<table id="table" class="export_table table table-striped table-bordered table-responsive-lg">
				...
				<tr class="filters">
					...
					<th class="text-alignment text-center">Total Paid Staff</th>
					<th class="text-alignment text-center">Total Paid Amount</th>
					<th width="15%" class="align-middle text-center">Actions</th>
				</tr>
				</thead>
				@permission('salary.expense')
				<tbody>
				@foreach($salaries as $date => $salary_group)
					...
					@endforeach
					<tr>
						<td class="text-center align-middle">{{date('F, Y', strtotime($salary->date))}}</td>
						<td class="text-center align-middle">{{count($salary_group)}}</td>
						<td class="text-center align-middle">{{$total}} RS</td>
						<td class="text-center">
							<a data-toggle="modal" data-target="#viewModal{{$salary->id}}">
	...
	@foreach($salaries as $date => $salary_group)
		...
		@foreach($salary_group as $salary)
			@php($total += intval($salary->paid_amount) )
		@endforeach
	<div class="modal fade" id="viewModal{{$salary->id}}">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">

				<div class="modal-header header-backgroud text-white">
					<h5 class="modal-title">Salary Report View</h5>
				...
				<div class="container">
					<a  target="_blank" name="gnrt_single" href="{{action('ExpenseController@downloadPDF', $salary->id)}}"
				...
				<div class="col-md-12 py-2 ">
					...
					<div class="card">
						<div class="card-header">
							<h5 class="mb-0">
								<div>
									<i class="fas fa-arrow-circle-down"></i> Salary Report View
								</div>
							</h5>
						</div>
						...
						<div class="card-body">
							<div class="row">
								...
								<div class="col-md-6">
									<p>{{date('F, Y', strtotime($salary->date))}}</p>
								</div>
							</div>
							<div class="row">
								<div class="col-md-6">
									<label>Total Paid Staff</label>
								</div>
								<div class="col-md-6">
									<p>{{count($salary_group)}}</p>
								</div>
							</div>
							<div class="row">
								<div class="col-md-6">
